...
SELECT a.fname, a.lname, aw.title
FROM  artist a
JOIN artwork aw
...
SELECT ci.Name AS 'City Name', 
		FORMAT(ci.Population, 0) AS 'City Population', 
        co.Name AS 'Country Name', 
        FORMAT(co.Population, 0) AS 'Country Population',
        co.Continent
FROM city ci
JOIN country co
	ON ci.CountryCode = co.Code
WHERE Continent IN ('North America', 'South America')
ORDER BY co.Population DESC, ci.Population DESC;
